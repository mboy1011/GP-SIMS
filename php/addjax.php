<?php
	...
	if ($_REQUEST['product']) {	
		...
		if ($result->num_rows>0) {
			...
			$sql = mysqli_query($db,"INSERT INTO tbl_salesdetails(sales_no,prod_id,quantity,amount) VALUES ('".intval($si_no)."','".floatval($prod)."','".intval($qty)."','".floatval($total)."')");
			$sql1 = mysqli_query($db,"UPDATE tbl_products SET quantity='$inventory' WHERE prod_id='$prod'");
			if (!$sql||!$sql1) {
				return false;
			}else{
				return true;
